...
const UpdateRPJ = ({ params }) => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true)
            const rpjByNotation = await rpjSv.getRPJByNotation(params.notation)
            ...
            const currentType = typesArray.find(t => t.name === rpjByNotation.typeName)
            setOptionType(currentType || null)

            // Preencher dados
            setData({
                register: rpjByNotation.register || "",
                service_type: currentType?.id || "",
                book: rpjByNotation.book || "",
                initial_sheet: rpjByNotation.initial_sheet || "",
                final_sheet: rpjByNotation.final_sheet || "",
                box: rpjByNotation.box || "",
                file_url: "",
                presenterName: rpjByNotation.presenterName || "",
                presenterDocument: rpjByNotation.presenterDocument || "",
                notation: rpjByNotation.notation || params.notation
            })

            setOriginalData(rpjByNotation)
            setHasFile(!!rpjByNotation.file)
    ...
    const handleUpdateRpjByNotation = async () => {
        ...
        try {
            setSaving(true)
            await rpjSv.updateRPJByNotation(params.notation, data)
            dispatch({
                type: SET_ALERT,
                        ...
                        >
                            ...
                            <Box sx={{ p: 3 }}>
                                ...
                                >
                                    ...
                                    <Button
                                        onClick={handleUpdateRpjByNotation}
                                        variant="contained"
                                        disabled={saving}
