...
import { FileText, Building2, MoreVertical, Hash } from "lucide-react";
            ...
            <Grid container spacing={2} sx={{
                ...
                {data.map((item, index) => (
                    <Grid item xs={12} sm={6} md={4} lg={3} key={item.number || index}>
                        ...
                        onClick={(e) => { handleClick(e); setNumber(item.number); }}
                        >
                            <Box className="card-stripe" sx={{
                                ...
                                <Tooltip title="Opcoes">
                                    <IconButton size="small" sx={{ color: "rgba(255,255,255,0.8)", "&:hover": { bgcolor: "rgba(255,255,255,0.15)" } }}
                                        onClick={(e) => { e.stopPropagation(); handleClick(e); setNumber(item.number); }}>
                            ...
                            <CardContent sx={{ p: 2 }}>
                                <Box sx={{ backgroundColor: alpha("#ef5350", 0.04), borderRadius: 1.5, p: 1.5 }}>
                                    <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mb: 0.5 }}>
                                        <Building2 size={13} color="#ef5350" />
